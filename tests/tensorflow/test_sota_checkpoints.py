...
class TestSotaCheckpoints(RunTest):
    ...
    def test_eval(self, sota_checkpoints_dir, sota_data_dir, eval_test_struct: EvalRunParamsStruct):
        ...
        if MODE == "TF2":
            cmd = (
                f"{sys.executable} examples/tensorflow/{sample_type}/{EVAL_SCRIPT_NAME_MAP[sample_type]} "
            ...
            cmd = (
                f"{sys.executable} examples/tensorflow/{sample_type}/main.py -m test "
        ...
        if eval_test_struct.reference_ is not None:
            ...
            reference_metric_file_path = pytest.metrics_dump_path / self.get_metric_file_name(
                eval_test_struct.reference_ + dataset_type_postfix
            )
    ...
    def test_openvino_eval(self, eval_test_struct: EvalRunParamsStruct, sota_checkpoints_dir, ov_data_dir, openvino):
        ...
        save_cmd = (
            f"{sys.executable} examples/tensorflow/{eval_test_struct.sample_type_}/{file}"
