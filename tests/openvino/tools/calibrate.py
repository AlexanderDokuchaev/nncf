...
class CustomJSONEncoder(json.JSONEncoder):
    def default(self, o):
        if isinstance(o, (nncf.TargetDevice, nncf.ModelType, nncf.QuantizationPreset)):
            return o.value
...
def map_ignored_scope(ignored):
    ...
    return {"ignored_scope": nncf.IgnoredScope(names=ignored.get("scope"), types=ignored.get("types"))}
...
def map_quantization_parameters(pot_parameters):
    ...
    for name in pot_parameters:
        if name in ignored_parameters or (
            name in default_parameters and pot_parameters[name] == default_parameters[name]
        ):
            continue
...
def quantize_model(xml_path, bin_path, accuracy_checcker_config, quantization_impl, quantization_parameters):
    ...
    if quantization_impl == "pot":
        ...
        quantized_model = quantize_native(ov_model, calibration_dataset, **quantization_parameters)
    else:
