...
def test_force_cuda_build(tmp_path):
    ...
    command = Command(
        "{} {}/extensions_build_checks.py {}".format(python_executable_with_venv, run_path, mode), path=run_path
    )
    command.run()

    version_command = Command(
        '{} -c "import torch; print(torch.__version__)"'.format(python_executable_with_venv), path=run_path
    )
    version_command.run()
