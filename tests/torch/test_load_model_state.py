...
MATCH_KEY_DESC_LIST = [
    ...
    MatchKeyDesc(
        num_loaded=0, is_resume=False, state_dict_to_load={"1": torch.zeros(1)}, model_state_dict={"1": torch.zeros(2)}
    ).size_mismatched(["1"]),
    MatchKeyDesc(
        num_loaded=1, is_resume=False, state_dict_to_load={"1": torch.zeros(1)}, model_state_dict={"1": torch.zeros(2)}
    )
    .keys_to_load(["2"])
    .model_keys(["2", "3"])
    .size_mismatched(["1"])
    ...
    .unexpected(["4"])
    .matched(["2"]),
    MatchKeyDesc(num_loaded=2).keys_to_load(["1", "2"]).model_keys(["1", "2"]).all_matched(),
    ...
    MatchKeyDesc(num_loaded=2).keys_to_load(["module.1", "nncf_module.2"]).model_keys(["1", "2"]).all_matched(),
    MatchKeyDesc(num_loaded=2).keys_to_load(["1", "2"]).model_keys(["module.1", "nncf_module.2"]).all_matched(),
    MatchKeyDesc(num_loaded=2)
    .keys_to_load(["module.nncf_module.1", "module.2"])
    .model_keys(["1", "nncf_module.2"])
    .all_matched(),
    MatchKeyDesc(num_loaded=0, expects_error=True)
    .keys_to_load(["module.nncf_module.1.1", "module.2"])
    .model_keys(["1", "2.2"])
    .all_not_matched(),
    ...
    .all_matched(),
    MatchKeyDesc(num_loaded=2).keys_to_load(["module.1", "1"]).model_keys(["module.1", "1"]).all_matched(),
    MatchKeyDesc(num_loaded=1, expects_error=True)
    .keys_to_load(["module.1", "1"])
    .model_keys(["module.1"])
    .matched(["module.1"])
    .unexpected(["module.1"]),
    MatchKeyDesc(num_loaded=2)
    ...
    .all_matched(),
    MatchKeyDesc(num_loaded=1, expects_error=True)
    .keys_to_load(["module.1"])
    .model_keys(["module.1", "1"])
    .matched(["1"])
    .missing(["module.1"]),
    MatchKeyDesc(num_loaded=1, expects_error=True)
    .keys_to_load(["1"])
    ...
    .missing(["module.1"]),
    MatchKeyDesc(num_loaded=1, expects_error=True)
    .keys_to_load(["1", "module.1"])
    .model_keys(["1"])
    .matched(["1"])
    .unexpected(["module.1"]),
    # can match new format of activation quantizer with |INPUT and |OUTPUT
    MatchKeyDesc(num_loaded=2)
    .keys_to_load(["relu_0." + OP1, "relu_0." + OP2])
    .model_keys(["relu_0|OUTPUT." + OP1, "relu_0|INPUT." + OP2])
    .all_matched(),
    ...
    .keys_to_load(
        [
            "module." + LEGACY_EXTERNAL_QUANTIZERS_STORAGE_PREFIX + ".relu_0." + OP1,
    ...
    .keys_to_load(
        [
            "module." + CURRENT_EXTERNAL_QUANTIZERS_STORAGE_PREFIX + ".relu_0." + OP1,
    ...
    .skipped(["1"]),
    MatchKeyDesc(num_loaded=0, expects_error=True)
    .keys_to_load(["module.nncf_module.1.1", "2.2"])
    .model_keys(["module.1", "module.2"])
    .keys_to_ignore(["1", "2.2"])
    .skipped(["module.1", "2.2"])
    .missing(["module.2"])
    .unexpected(["module.nncf_module.1.1"]),
