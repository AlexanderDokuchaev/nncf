...
from tests.torch.experimental.hook_executor_mode.helpers import get_wrapped_simple_model_with_hook


@pytest.fixture(scope="function")
def example_input() -> torch.Tensor:
    return torch.ones((10, 1, 3, 3))


def run_one_epoch(model: nn.Module, optimizer: torch.optim.Optimizer):
    pred: torch.Tensor = model(torch.ones((10, 1, 3, 3)))
...
def test_train_simple():
    ...
    run_one_epoch(wrapped_model, optimizer)
...
def test_train_data_parallel():
    ...
    run_one_epoch(parallel_model, optimizer)
    assert all(p.grad is not None for p in wrapped_model.parameters())
