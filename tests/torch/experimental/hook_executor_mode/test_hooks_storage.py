...
from tests.torch.experimental.hook_executor_mode.helpers import CallCount
...
def test_insert(hook_type: HookType):
    hooks = HookStorage()
    module = nn.Identity()
    hooks.insert_hook(hook_type, "group", "foo", 0, module)
    assert hooks.storage[hook_type.value]["group"]["foo"]["0"] is module
...
def test_execute_priority(hook_type: HookType, group_names: List[str]):
    ...
    for group_name in group_names:
        ...
        hooks.insert_hook(hook_type, group_name, "foo", 0, module)
    hooks.execute_hook(hook_type, "foo", 0, torch.tensor(1))
...
def test_remove_group(hook_type: HookType):
    ...
    with pytest.raises(KeyError):
        hooks.storage[hook_type.value]["group"]["foo"]["0"]
    hooks.execute_hook(hook_type, "foo", 0, torch.tensor(1))
    assert module.call_count == 1
