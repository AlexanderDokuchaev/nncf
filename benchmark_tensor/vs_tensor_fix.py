...
def tune_range(left_border: OVNNCFTensor, right_border: OVNNCFTensor, num_bits, unify_zp=False):
    ...
    with np.errstate(invalid="ignore", divide="ignore"):
        ra = backend.where(qval < level_high, (qval / (qval - level_high) * right_border).tensor, left_border.tensor)
        rb = backend.where(qval > 0.0, ((qval - level_high) / qval * left_border).tensor, right_border.tensor)
...
profiler.dump_stats("vs_tensor_fix.prof")
