...
class InsertionPointGraph(nx.DiGraph):
    ...
    def get_ip_graph_with_merged_hw_optimized_operations(
        self, full_fusing_pattern: GraphPattern, known_non_constant_node_keys: Optional[List[str]] = None
    ) -> "InsertionPointGraph":
        """
        ...
        if known_non_constant_node_keys is not None:
            start_traversing_node_keys = [
                node.node.data[NNCFGraph.KEY_NODE_ATTR] for node in known_non_constant_node_keys
            ]
