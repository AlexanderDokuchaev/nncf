...
from nncf.experimental.torch.hook_executor_mode.wrapper import remove_group


def create_fns(fns, name):
    """
    ...
    def combine_fns(x) -> torch.Tensor:
        """
        ...
        """
        if name == "_model:layer1:0:bn2/batch_norm/0":
            print(name, len(fns))
        for f in fns:
            f(x)
        return x
...
class PTStatisticsAggregator(StatisticsAggregator):
    HOOKS_GROUP_NAME = "01_statistics_hooks"

    def collect_statistics(self, model: nn.Module, graph: NNCFGraph) -> None:
        with torch.no_grad():
            super().collect_statistics(model, graph)
        remove_group(model, self.HOOKS_GROUP_NAME)
        # model.hooks.remove_group(self.HOOKS_GROUP_NAME)
        # model.nncf.remove_hooks_group()
        ...
        for _statistic_points in statistic_points.values():
            for _statistic_point in _statistic_points:
                ...
                for collectors in _statistic_point.algorithm_to_tensor_collectors.values():
                    fns = []
                    for collector in collectors:
                        fns.append(create_register_input_hook(collector=collector))

                node_name = _statistic_point.target_point.target_node_name
                print(f"AAA {node_name}")

                transformation_commands.append(
                    PTInsertionCommand(
                        _statistic_point.target_point,
                        create_fns(fns, str(node_name)),
                        TransformationPriority.FP32_TENSOR_STATISTICS_OBSERVATION,
                        hooks_group_name=self.HOOKS_GROUP_NAME,
                    )
                )
