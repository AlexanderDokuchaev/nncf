...
class StructuredMaskRule:
    ...
    def __init__(self, keywords: Union[List[str], str], prune_by_row: bool, prune_grid: Tuple[int, int]):
        """
    ...
    def __str__(self) -> str:
        return "{class_name}({attrs})".format(
            class_name=self.__class__.__name__, attrs=", ".join(f"{k}={v}" for k, v in self.__dict__.items())
        )
    ...
    def from_compressed_model(cls, compressed_model: NNCFNetwork):
        ...
        if any(dim != dim_per_head_list[0] for dim in dim_per_head_list[1:]):
            raise NotImplementedError(
                "Currently we only support SwinTransformers " "whose attention heads all have the same dimension."
            )
