...
class MinMaxQuantizationParameters(AlgorithmParameters):
    ...
    def __init__(
        ...
        q_weight_constraints = self._get_quantizer_constraints(
            QuantizerGroup.WEIGHTS, preset, weight_bits, weight_granularity, signed_weights
        )
        q_activation_constraints = self._get_quantizer_constraints(
            QuantizerGroup.ACTIVATIONS, preset, activation_bits, activation_granularity, signed_activations
        )
...
class MinMaxQuantization(Algorithm):
    ...
    def _get_stat_collector(
        ...
        def is_default_parameters(range_type: RangeType, quantizer_config: QuantizerConfig) -> bool:
        ...
        if range_type == RangeType.MINMAX:
            return self._backend_entity.minmax_statistic_collector(
                nncf_graph, target_point, quantizer_config, num_samples=self._parameters.number_samples
            )
        if range_type == RangeType.MEAN_MINMAX:
            return self._backend_entity.mean_minmax_statistic_collector(
                nncf_graph,
                target_point,
                quantizer_config,
    ...
    def _get_quantizer_setup(self, nncf_graph: NNCFGraph, pattern: GraphPattern) -> SingleConfigQuantizerSetup:
        ...
        ignored_names = set(
            ignored_names + get_ignored_node_names_from_ignored_scope(model_type_ignore_scope, nncf_graph, strict=False)
        )
        ...
        weighted_node_and_qconf_lists = assign_qconfig_lists_to_modules(
            ...
            global_weight_constraints=self._parameters.global_quantizer_constraints[QuantizerGroup.WEIGHTS],
        ...
        inference_nncf_graph = transform_to_inference_graph(
            deepcopy(nncf_graph), shapeof_metatypes=self._backend_entity.shapeof_metatypes
        )
        ...
        solver = QuantizerPropagationSolver(
            ignored_scopes=ignored_names,
            target_scopes=None,
            hw_config=hw_config,
            default_trait_to_metatype_map=self._backend_entity.quant_trait_op_dict,
            default_qconfig_list=[
                self._get_default_qconfig(self._parameters.global_quantizer_constraints[QuantizerGroup.ACTIVATIONS])
            ],
