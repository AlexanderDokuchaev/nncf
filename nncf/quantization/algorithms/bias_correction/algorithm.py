...
class BiasCorrectionParameters(AlgorithmParameters):
    ...
    def __init__(self, number_samples: int = 100, threshold: float = 1000, inplace_statistics: bool = True) -> None:
        """
        ...
        for node in nodes_with_bias:
            ...
            if node_name in biased_after_input_nodes:
                ...
                stat_collector = self._backend_entity.batch_statistic_collector(
                    num_samples=self.number_samples, inplace=self.inplace_statistics
                )
            ...
            stat_collector = self._backend_entity.mean_statistic_collector(
                reduction_shape=channel_axis, num_samples=self.number_samples, inplace=self.inplace_statistics
            )
            statistic_container.add_statistic_point(
