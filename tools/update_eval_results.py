...
TORCH_SAMPLE_TYPE_TO_DESCRIPTOR = {
    "classification": SampleDescriptor(
        ...
        sub_tables_in_own_readme=[
            ...
            SampleReadmeSubTableDescriptor(
                ...
                models_duplicated_from_main_table=[
                    "resnet50_imagenet",
                    "resnet18_imagenet",
                    "resnet34_imagenet",
                    "googlenet_imagenet",
    ...
    "semantic_segmentation": SampleDescriptor(
        ...
        sub_tables_in_own_readme=[
            SampleReadmeSubTableDescriptor(
                anchor='<a name="filter_pruning"></a>',
                model_names=["unet_mapillary", "unet_mapillary_pruning_geometric_median"],
    ...
    "object_detection": SampleDescriptor(
        ...
        sub_tables_in_own_readme=[
            SampleReadmeSubTableDescriptor(
                anchor='<a name="filter_pruning"></a>',
                model_names=["ssd300_vgg_voc", "ssd300_vgg_voc_pruning_geometric_median"],
...
def get_fp32_and_compressed_metrics(
    model_name_to_metric_dict: Dict, model_name: str, reference: str, target: str
...
def get_results_table_rows(
    per_sample_config_dict: Dict, model_name_to_metric_dict: Dict, sample_type: str, table_format: str, framework: str
    ...
    for data_name_ in per_sample_config_dict:
        ...
        for model_name in model_dicts:
            ...
            fp32_ref_metric, compressed_metric = get_fp32_and_compressed_metrics(
                model_name_to_metric_dict, model_name, reference, target
            )
            ...
            model_name_vs_row[model_name] = build_per_model_row(
                table_format,
                model_display_name,
                compression,
                dataset_name,
    ...
    if table_format == "overview":
        ...
        retval[RESULTS_ANCHOR_IN_SAMPLE_OWN_README] = [
            header,
        ] + list(model_name_vs_row.values())
        return retval


def update_target_metrics_and_thresholds(config_dict: Dict, model_name_to_metric_dict: Dict, framework: str) -> Dict:
        ...
        for dataset_name in config_dict[sample_name]:
            ...
            for model_name, model_dict in models_for_dataset_dict.items():
                ...
                if "diff_fp32_min" in model_dict:
                    ...
                    actual_diff_fp32 = model_name_to_metric_dict[model_name] - model_name_to_metric_dict[ref_model_name]
                    ...
                    if actual_diff_fp32 < diff_fp32_min_value:
                        print(
                            f"Warning: model {model_name} scores less ({actual_diff_fp32}) "
                ...
                if "diff_fp32_max" in model_dict:
                    ...
                    if actual_diff_fp32 > diff_fp32_max_value:
                        print(
                            f"Warning: model {model_name} scores more ({actual_diff_fp32}) "
...
def get_sample_desc(sample_type: str, framework: str) -> SampleDescriptor:
    framework_sample_type_to_desc = (
        TORCH_SAMPLE_TYPE_TO_DESCRIPTOR if framework == "torch" else TF_SAMPLE_TYPE_TO_DESCRIPTOR
    )
...
def main(argv):
    ...
    parser.add_argument(
        "--framework",
        "-f",
        help="The framework for which the eval results are to be updated",
        choices=["tf", "torch"],
        required=True,
    )
    parser.add_argument(
        "--results", "-r", help="A metrics.json file from a latest checkpoint evaluation run", required=True
    )
    parser.add_argument(
        "--config", "-c", help="A .json file with definitions of tested checkpoints (sota_checkpoints_eval.json)"
    )
    parser.add_argument(
        "--output",
        "-o",
    ...
    for sample_type in sota_checkpoints_eval:
        anchor_vs_table_rows = get_results_table_rows(
            sota_checkpoints_eval[sample_type],
            measured_metrics,
            sample_type,
        ...
        for anchor, table_rows in anchor_vs_table_rows.items():
            ...
            if args.inplace:
                ...
                update_table_inplace(table_str, sample_desc.path_to_readme, anchor)
            else:
    ...
    for sample_type in sota_checkpoints_eval:
        ...
        if args.inplace:
            ...
            table_str = table_str.lstrip("\n")
            update_table_inplace(table_str, PROJECT_ROOT / "README.md", sample_desc.result_table_anchor_in_main_readme)
        else:
            write_table_to_md_file(mdfile, table_rows)
