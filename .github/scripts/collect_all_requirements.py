...
def _run_command(command: list[str]) -> str:
    ...
    if result.returncode != 0:
        msg = f"Command failed ({' '.join(command)}): {result.stderr.strip() or result.stdout.strip()}"
...
def collect_requirements_files() -> list[Path]:
    output = _run_command(["git", "ls-files", "**/requirements*.txt", "requirements.txt"])
    paths = {Path(line) for line in output.splitlines() if line.strip()}
    print(f"Found {len(paths)} requirements.txt files")
    return sorted(paths)


def get_unique_packages_from_requirements_files() -> set[str]:
    unique_packages = set()
    ...
    return set(sorted(unique_packages))


def get_package_requirements() -> set[str]:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collect tracked requirements.txt files from a git repository")
    parser.add_argument(
        "--output",
        type=Path,
        default=None,
        help="Optional output file. If omitted, paths are printed to stdout.",
    )
